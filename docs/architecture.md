...
- **2026-07-23** — Live at https://rubik-cube-solver-94w4m.ondigitalocean.app (App Platform app `rubik-cube-solver`, id `01a6bbb4-9ce1-41b9-959b-3da1237c6cb1`). Verified end-to-end against the real deployment: `/`, `/api/health`, and a real `/api/coach/chat` round-trip through DO Serverless Inference all confirmed working, and the production JS bundles were checked to confirm no `localhost` URL leaked in (the relative-URL fix below actually took effect at build time). `basic-xxs` instance size on both components (~$10/month total).
- **2026-07-23** — Deployed to DigitalOcean App Platform as one app with two components sharing a single public domain via path-based ingress routing (`/api/*` → backend with `preserve_path_prefix: true`, `/` → frontend) instead of two separately-hosted apps. This avoids CORS entirely in production, since the browser sees same-origin requests. Changed `frontend/src/lib/backend.ts` to default `NEXT_PUBLIC_BACKEND_URL` to `''` (relative) rather than `http://localhost:8000`, since that default is now only correct for local dev, where it's supplied explicitly via `.env.local` (gitignored, mirrors `.env.local.example`) because the two local dev servers are genuinely on different origins. `DO_INFERENCE_API_KEY` is set as an App Platform `SECRET`-type env var (encrypted at rest, backend component only); `FRONTEND_ORIGIN` uses the bindable `${APP_URL}` variable instead of a hardcoded domain.
